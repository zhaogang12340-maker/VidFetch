...
## ★ 首次安装后请重启一次浏览器（重要）
**第一次装好后，请完全关闭并重新打开 Chrome / Edge，再使用右键下载。**

原因（一次性，重启后永久生效）：
- **MV3 后台 Service Worker 首次激活**：扩展刚装上时，浏览器对“首个右键点击”常常唤不醒后台脚本，表现为**点了没任何反应**。重启后才正常。
- **原生宿主注册时机**：若你是在浏览器**已打开**时运行的 `install_extension.bat`，浏览器要重启后才认得新注册的宿主。

> 这是浏览器（Chromium）对刚安装扩展的固有行为，不是本扩展的问题；**只需重启一次**，之后右键下载都正常，不会再出现。
...
## 常见问题
- **首次安装后点右键没反应**：这是正常的一次性现象，**完全重启一次 Chrome/Edge 即可**（原因见上方「★ 首次安装后请重启一次浏览器」）。
- **点了没反应 / 提示“无法连接 VidFetch”**：先运行过 `install_extension.bat` 了吗？换过目录要再跑一次；跑完同样**重启一次浏览器**。
